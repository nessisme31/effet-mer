...
interface Props {
  price: number                               // ← prix total calculé (déjà multiplié si multi-jets)
  activityLabel?: string                      // ← ex: "Jet Ski VX × 2 · 30 min"
...
export default function Step5Payment({ price, activityLabel, onNext, onBack }: Props) {
    ...
    <div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 mb-6 text-center">
        ...
        <p className="text-4xl font-bold text-blue-700">
          {price.toLocaleString()} <span className="text-2xl">{CONFIG.currency}</span>
        </p>
        {activityLabel && (
          <p className="text-gray-500 text-sm mt-2">{activityLabel}</p>
        )}
      </div>
