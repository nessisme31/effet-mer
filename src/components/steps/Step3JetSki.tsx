...
export default function Step3JetSki({
  jetType, clientFirstname, clientName, onNext, onBack, onAddToWaitingList
}: Props) {
            ...
            <button
              ...
              onClick={() => {
                if (isOccupied) {
                  ...
                  setSelected(selected === jet.id ? '' : jet.id)
      ...
      {allOccupied && !pendingWaiting && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 text-center mb-4">
          ...
          <p className="text-red-500 text-sm mt-1">
            Appuyez sur un jet pour mettre le client en file d'attente
          </p>
        </div>
      )}
        ...
        <div className="bg-orange-50 border-2 border-orange-300 rounded-2xl p-4 mb-4">
            ...
            <div>
              ...
              {rentalMap[pendingWaiting] && (
                <p className="text-orange-600 text-sm">
                  ...
                  {' · '}
                  Client actuel : {rentalMap[pendingWaiting].clientFirstname}{' '}
                  {rentalMap[pendingWaiting].clientName}
                </p>
              )}
            </div>
          </div>
          <div className="bg-white rounded-xl p-3 mb-3">
            <p className="text-gray-600 text-sm">
              Ajouter <strong>{clientFirstname} {clientName}</strong> en file d'attente pour{' '}
              <strong>{pendingWaiting}</strong> ?
            </p>
